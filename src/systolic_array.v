module systolic_array #(
  ...
  input wire en,
  input wire drain,
  input wire [$clog2(32)-1:0] drain_cnt,
  ...
  wire signed [DATA_WIDTH-1:0]   a_wire [0:ARRAY_SIZE*(ARRAY_SIZE+1)-1];
  ...
  function integer idx_a;
    ...
    begin idx_a = row * (ARRAY_SIZE + 1) + col; end
  endfunction

  function integer idx_b;
    input integer row, col;
    begin idx_b = row * ARRAY_SIZE + col; end
  endfunction

  function integer idx_c;
    input integer row, col;
    begin idx_c = row * ARRAY_SIZE + col; end
  endfunction
  ...
  generate
    for(i = 0; i < ARRAY_SIZE; i = i + 1) begin : input_connections
      ...
      assign c_wire[idx_c(0, i)] = 0;   // top boundary: no partial sum from above
    end
  endgenerate
